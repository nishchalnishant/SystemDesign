...
class ShardRouter:
    def __init__(self, dsns: list[str]):
        # one connection per shard
        self._shards = [psycopg2.connect(dsn) for dsn in dsns]

    def get_shard_for(self, user_id: int):
        """Hash-based routing: consistent across all nodes."""
        shard_index = abs(user_id) % len(self._shards)
        return self._shards[shard_index]

    def get_shard_by_range(self, user_id: int):
        """Range-based routing: shard 0 = [0, 1M), shard 1 = [1M, 2M), etc."""
        shard_index = min(user_id // 1_000_000, len(self._shards) - 1)
        return self._shards[shard_index]

# Usage in repository:
def find_by_id(user_id: int) -> dict[str, Any]:
    conn = shard_router.get_shard_for(user_id)
...
# SortedList as a consistent hash ring
...
class ConsistentHashRing:
    def __init__(self):
        self._ring: dict[int, Any] = {}   # hash → shard connection
        self._sorted_keys: list[int] = []

    def add_shard(self, shard, virtual_nodes: int = 150) -> None:
        for i in range(virtual_nodes):
            h = _hash(f"{shard}-{i}")
            self._ring[h] = shard
            bisect.insort(self._sorted_keys, h)

    def get_shard_for(self, key: str):
        h = _hash(key)
