...
### Java TreeMap Ring Implementation
...
public class ConsistentHashRing<T> {
    ...
    public synchronized T getNode(String key) {
        ...
        long hash = hash(key);
        // Walk clockwise: find first node at or after hash
        SortedMap<Long, T> tail = ring.tailMap(hash);
        Long nodeHash = tail.isEmpty() ? ring.firstKey() : tail.firstKey();
        return ring.get(nodeHash);
    }

    // Returns the R nodes that should hold replicas of this key
    public synchronized List<T> getReplicaNodes(String key, int replicationFactor) {
        ...
        SortedMap<Long, T> tail = ring.tailMap(hash);
        Iterator<T> iterator = Iterables.concat(tail.values(), ring.values()).iterator();
        while (replicas.size() < replicationFactor && iterator.hasNext()) {
            T node = iterator.next();
            if (seen.add(node)) replicas.add(node); // skip duplicate vnodes of same physical node
        }
        return replicas;
    }
    ...
    private long hash(String key) {
        ...
        // Use first 8 bytes as a long for a wider ring
        long h = 0;
        for (int i = 0; i < 8; i++) {
            h = (h << 8) | (digest[i] & 0xFF);
        }
...
```java
ConsistentHashRing<String> ring = new ConsistentHashRing<>(150); // 150 vnodes/node
ring.addNode("cache-node-1:6379");
ring.addNode("cache-node-2:6379");
ring.addNode("cache-node-3:6379");

String node = ring.getNode("user:12345:profile");  // → "cache-node-2:6379"

// Simulate node failure
ring.removeNode("cache-node-2:6379");
String failoverNode = ring.getNode("user:12345:profile"); // → "cache-node-3:6379"
// Only ~1/3 of keys remapped, rest unaffected
```

### Load Distribution Verification

```java
// Quick sanity check: measure key distribution across nodes
...
distribution.forEach((node, count) ->
    System.out.printf("%-30s → %d (%.1f%%)%n", node, count, count / 1000.0));
// With 150 vnodes, expect ~33% ± 2% per node for 3-node cluster
```
