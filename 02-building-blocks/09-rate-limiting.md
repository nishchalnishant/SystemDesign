...
| Algorithm | Boundary Spike | Burst Control | Space |
|---|---|---|---|
| Fixed Window | ❌ 2× at boundary | ❌ No | O(1) |
| Token Bucket | ✅ None | ✅ Configurable | O(1) |
| Leaky Bucket | ✅ None | ✅ No burst | O(queue) |
| Sliding Window | ✅ None | ✅ Configurable | O(N) per key |

**Recommendation**: Token Bucket for user-facing APIs; Sliding Window for billing/financial.
...
### Token Bucket (Python, thread-safe)
...
limiter = TokenBucket(capacity=10, refill_rate=1.0)  # 10 burst, 1/sec sustained
```

### Sliding Window in Redis (Lua, language-agnostic)

```lua
-- KEYS[1] = rate_limit:{user_id}
-- ARGV[1] = window_start (now - window_sec * 1000), ARGV[2] = now, ARGV[3] = limit
