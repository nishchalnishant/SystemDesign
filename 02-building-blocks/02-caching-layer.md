...
def update_user_profile(user_id: str, updated: dict) -> None:
...
2. **Local L1 cache**: Keep a tiny in-process cache (e.g. an in-process dict/OrderedDict) for the hottest keys with a 1–5s TTL. Reads never leave the application server for those keys. The database sees near-zero reads for them.
...
class TTLCache:
    """Simple in-process L1 cache with TTL — hot key buffer in front of Redis."""

    def __init__(self, max_size: int = 1_000, ttl: float = 5.0):
...
local_cache = TTLCache(max_size=1_000, ttl=5.0)

def get_profile(user_id: str) -> dict:
    key = f"user:profile:{user_id}"
    hit = local_cache.get(key)              # L1: in-process
    if hit is not None:
        return hit
    profile = redis_client.get_profile(key) # L1 miss → L2 Redis
    local_cache.set(key, profile)
    return profile
```
