...
producer.produce("order-tasks", key=order_id, value=json.dumps(order))
producer.flush()
...
consumer.subscribe(["order-tasks"])

def process_order(order_json: str) -> None:
    order = json.loads(order_json)
    if order_repo.is_processed(order["id"]):
        return                          # dedup
    kitchen_service.cook(order)
    order_repo.mark_processed(order["id"])

while True:
    msg = consumer.poll(timeout=1.0)
    if msg is None or msg.error():
        continue
    process_order(msg.value().decode())
    consumer.commit()
```
...
with db.transaction():
    order_repo.save(order)                                              # main record
    outbox_repo.save(topic="order-created", entity_id=order_id,        # outbox
                     payload=json.dumps(order))

# Relay process (runs separately):
def relay_outbox() -> None:
    pending = outbox_repo.find_unpublished()
    for event in pending:
        producer.produce(event.topic, value=event.payload)
        producer.flush()
        outbox_repo.mark_published(event.id)
```
