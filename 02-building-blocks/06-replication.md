...
class UserRepository:
    def __init__(self, primary_dsn: str, replica_dsn: str):
        self._primary = psycopg2.connect(primary_dsn)   # leader
        self._replica = psycopg2.connect(replica_dsn)   # follower

        # After a write, this user needs to read from primary
        # to avoid seeing stale data (replication lag)
        self._recent_writers: set[str] = set()
        self._lock = threading.Lock()

    def _expire_writer(self, user_id: str, delay: float = 5.0):
        """Remove after 5 seconds — enough for async replication to catch up."""
        def _remove():
            time.sleep(delay)
            with self._lock:
                self._recent_writers.discard(user_id)
        threading.Thread(target=_remove, daemon=True).start()

    def update_profile(self, user_id: str, profile: dict) -> None:
        with self._primary.cursor() as cur:
            cur.execute("UPDATE users SET profile = %s WHERE id = %s",
                        (profile, user_id))
        self._primary.commit()
        with self._lock:
            self._recent_writers.add(user_id)
        self._expire_writer(user_id)

    def get_profile(self, user_id: str) -> Optional[dict]:
        # Route to primary if this user just wrote (read-your-writes guarantee)
        with self._lock:
            conn = self._primary if user_id in self._recent_writers else self._replica
...
select = session.prepare("SELECT * FROM users WHERE id = ?")
select.consistency_level = CL.QUORUM
row = session.execute(select, (user_id,)).one()
```
