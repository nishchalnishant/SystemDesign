...
def jwt_auth(token: str) -> dict:
    """Concierge checks the room key — raises if invalid."""
    try:
...
def rate_limit(user_id: str, limit: int = 100) -> bool:
    """100 req/s per user via Redis sliding counter."""
    key = f"rl:{user_id}"
    count = r.incr(key)
    if count == 1:
        r.expire(key, 1)
...
def route_request(path: str, method: str, headers: dict, body: bytes):
    """Route to the appropriate backend service."""
    for prefix, upstream in ROUTES.items():
        if path.startswith(prefix):
            url = upstream + path
            resp = requests.request(method, url, headers=headers, data=body, timeout=2)
