...
Think of a library call number system. A book might have the full title *"Introduction to Algorithms, Third Edition by Thomas H. Cormen et al."* but on the shelf it lives at *"005.1 COR"*. The call number is short, memorable, and maps back to exactly one shelf location. You use the short code; the librarian finds the full location.

A URL shortener works the same way: the long URL is the book title, the short code is the call number, and the database is the library catalog. The redirect service is the librarian.
