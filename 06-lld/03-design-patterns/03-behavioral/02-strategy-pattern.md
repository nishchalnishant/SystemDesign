...
class StripeStrategy(PaymentStrategy):
    def pay(self, amount): ...  # Stripe logic

class PayPalStrategy(PaymentStrategy):
    def pay(self, amount): ...  # PayPal logic
```
...
class ShoppingCart:
    def __init__(self):
        self._strategy = None  # PaymentStrategy, set before checkout
...
- "What is Strategy and when do you use it instead of if/elif?" → Strategy: encapsulate each algorithm in its own class behind a common interface; pass the desired algorithm as a dependency. Replace `if payment_type == 'credit': ... elif payment_type == 'paypal': ...` with `payment_strategy.process(amount)`. Use Strategy when: the algorithm varies independently from clients that use it, you have multiple conditional branches selecting algorithm variants, you want to add new algorithms without modifying existing code (OCP).
- "Strategy vs Template Method — how do you choose?" → Template Method: defines the algorithm skeleton in a base class, with specific steps overridden by subclasses. Uses inheritance. Good when the algorithm structure is fixed, only steps vary. Strategy: defines the whole algorithm in an interchangeable object. Uses composition. Good when the entire algorithm is swappable. Key question: "Is the algorithm structure fixed?" → yes → Template Method. "Should the entire algorithm be replaceable?" → yes → Strategy.
- "How do you make Strategy configurable at runtime vs compile time?" → Runtime: inject the strategy at construction time (`OrderService(payment_strategy=CreditCardStrategy())`) or via a setter. The strategy can be changed without restarting the application (load from DB config, swap on feature flag). Compile time: subclasses hard-code the strategy via Template Method — not swappable at runtime. For runtime configurability: use Strategy with a registry (factory maps string → strategy instance).
- "Your Strategy implementations share 30% identical code. How do you handle this?" → Extract the shared code to a utility/helper class that all strategies use (composition). Don't extract it to a base class (AbstractStrategy) unless the shared behavior is intrinsic to the strategy concept — that path leads to inheritance creep. Alternatively, the shared code may indicate the wrong boundary: maybe it belongs in the caller, not the strategy.
