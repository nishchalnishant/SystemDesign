...
class NotificationSender(ABC):
    ...
    def _pre_send_audit_log(self, to, formatted):
        print(f"Logging before send: {formatted} to {to}")

    # Abstract: subclasses must implement custom message composition
    @abstractmethod
    def _compose_message(self, formatted_message): ...
    ...
    def _post_send_analytics(self, to):
        print(f"Analytics updated for: {to}")
...
- "What does Template Method enforce that Strategy doesn't?" → Template Method enforces the algorithm skeleton — the sequence and structure of steps are fixed in the base class; only the specific step implementations are customizable. Strategy makes the entire algorithm swappable — the caller can replace the whole algorithm. Use Template Method when "the steps and their order are fixed, but the implementation of some steps varies." Use Strategy when "the entire algorithm is replaceable."
- "Why is Template Method sometimes considered an anti-pattern?" → It couples behavior to inheritance. Every variation requires a subclass. If the number of variations grows, you get subclass explosion. The base class becomes a "god class" that owns the algorithm structure plus the default implementations. Hard to test (must subclass to test different paths). Modern preference: prefer Strategy (composition) over Template Method (inheritance) — more flexible and testable.
- "How would you convert a Template Method design to Strategy?" → Extract each abstract method into its own functional interface (or Strategy). The base class becomes a context: `ReportGenerator(formatter: Formatter, validator: Validator)`. `generate()` calls `validator.validate(data)` and `formatter.format(data)` instead of `this.validate()` / `this.format()`. Now you compose behaviors instead of inheriting them — fully configurable at runtime.
- "Give a real example of Template Method in production code." → Java's `AbstractList.iterator()` defines the iteration algorithm using `get(index)` and `size()` as abstract methods — subclasses implement just those two methods. Java's `HttpServlet.service()` dispatches to `doGet()` / `doPost()` (abstract methods you override). JUnit's `setUp()` / `test()` / `tearDown()` lifecycle. Android's `Activity.onCreate()` / `onResume()` / `onPause()` lifecycle. These are all Template Method in the standard library — the framework owns the orchestration.
