...
class IdleState(State):
    ...
    def insert_coin(self):    self._machine.set_state(self._machine.has_coin_state)
    def select_product(self): print("Insert coin first")
...
class HasCoinState(State):
    ...
    def select_product(self): self._machine.set_state(self._machine.dispensing_state)
    def dispense(self):       print("Select product first")
```
...
class VendingMachine:
    def __init__(self):
        self._current_state = None  # State, assigned after states are created

    def insert_coin(self):    self._current_state.insert_coin()
    def select_product(self): self._current_state.select_product()
    def dispense(self):       self._current_state.dispense()

    def set_state(self, s): self._current_state = s
```
...
class VendingMachine:
    def __init__(self):
        ...
        self._current_state = self.idle_state  # start in idle

    def set_state(self, state):
        self._current_state = state

    def insert_coin(self):  self._current_state.insert_coin()
    def press_button(self): self._current_state.press_button()
...
- "State vs Strategy — they look identical in code. How do they differ in intent?" → Structure is nearly identical (context holds a reference to an interchangeable object; context delegates to it). Intent differs: Strategy = different algorithms for the same operation (the algorithm is chosen externally). State = different behavior based on the object's current state, with state transitions managed internally. Key difference: in State, the state objects know about each other and may trigger transitions. In Strategy, strategies are independent and don't know about each other.
- "Give a concrete example where State is better than a switch statement." → Order lifecycle: `PLACED → PAID → SHIPPED → DELIVERED → CANCELLED`. A `switch(this.status)` in every method (`cancel()`, `ship()`, `refund()`) spreads transition logic everywhere. State pattern: each `OrderState` class (`PlacedState`, `PaidState`, etc.) implements the allowed operations and throws `IllegalStateException` for invalid operations. `PlacedState.cancel()` transitions to `CancelledState`; `PaidState.cancel()` triggers a refund then cancels. Transitions are self-contained and verifiable.
- "How do you enforce valid state transitions?" → State objects only expose valid operations. `PaidState` has no `ship()` method (or throws). The context delegates — it doesn't check state validity itself. To make invalid transitions impossible at compile time (stronger guarantee): use a type state pattern (separate Java types for each state, method signatures only accept the right state). For dynamic enforcement: store valid next states per state and validate at transition time.
- "Where does the transition logic live — in the context or the state?" → Either is valid; the State pattern literature shows both. Preferred: state decides transitions. `PlacedState.pay() → context.setState(new PaidState())`. This keeps each state's logic self-contained. If the context decides: you're back to centralized conditional logic, defeating the pattern. Exception: if the transition condition depends on context data (e.g., `if context.payment_amount > 1000: setState(ManualReviewState())`), the context may need to participate.
