...
class LazyReportProxy(ReportGenerator):
    def __init__(self):
        self._real = None  # nothing loaded yet
...
class ProxyImage(Image):
    def __init__(self, filename):
        self._filename = filename
        self._real = None  # None until actually needed; no disk IO here
...
- "What are the three main uses of a Proxy and how do they differ?" → (1) Virtual Proxy (lazy initialization): the real object is expensive to create; the proxy creates it on first use. (2) Protection Proxy: controls access — the proxy checks permissions before delegating. (3) Remote Proxy: the real object is on another machine; the proxy handles network communication transparently. All three share the same interface as the real object — callers can't tell they're talking to a proxy.
- "How does a caching proxy work? Give a concrete example." → The proxy intercepts `get(key)` calls and checks its cache before delegating to the real service. `result = cache.get(key); if result is None: result = real_service.get(key); cache.set(key, result); return result`. The caller calls `service.get(key)` on the proxy — identical interface. This is exactly what Spring's `@Cacheable` annotation does: it wraps the method with a caching proxy at runtime (via AOP / bytecode proxy).
- "Proxy vs Decorator — how do you tell them apart?" → Same structural implementation (both wrap an object with the same interface). Different intent: Decorator adds behavior that the client knows about (logging, caching — behavioral augmentation). Proxy controls access to the real object, often without the client's knowledge (lazy init, access control, remote stub). Practical rule: if the wrapping is transparent and the caller doesn't know about it — it's a Proxy. If the caller deliberately wraps to add behavior — it's a Decorator.
- "How does Python's `__getattr__` enable a Proxy?" → By overriding `__getattr__`, a proxy class can intercept any attribute access and delegate to the wrapped object dynamically: `def __getattr__(self, name): return getattr(self._real, name)`. This means the proxy doesn't need to explicitly define every method of the wrapped interface — it forwards all attribute lookups automatically. Used in ORM lazy loading (SQLAlchemy's `LazyLoad` proxy) and mock frameworks.
