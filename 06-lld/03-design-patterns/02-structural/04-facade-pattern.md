...
def place_order(self, order, user):
    if not self._inventory_service.is_available(order.product_id, order.qty):
        raise OutOfStockException()
    self._inventory_service.reserve(order.product_id, order.qty)

    result = self._payment_service.charge(user.payment_method, order.total)
    if not result.success:
        raise PaymentFailedException()

    invoice_id = self._invoice_service.generate(order, user)

    self._shipping_service.schedule_delivery(order, user.address)

    self._loyalty_service.add_points(user.id, order.total)
```
...
class OrderFacade:
    ...
    def place_order(self, order, user):
        ...
        self._inventory.reserve(order.product_id, order.qty)
        self._payment.charge(user.payment_method, order.total)
        self._invoice.generate(order, user)
        self._shipping.schedule_delivery(order, user.address)
        self._loyalty.add_points(user.id, order.total)
```
...
class Projector:
    ...
    def set_resolution(self, r): print(f"Projector Resolution: {r}")
...
class OrderController:
    def place_order(self, order):
        ...
        self._payment.charge(order.customer, order.total)
        self._warehouse.pick_and_pack(order)
        self._shipping.schedule_pickup(order)
        self._notification.send_confirmation(order)
        self._loyalty.add_points(order.customer, order.total)
...
- "What does Facade provide that just calling subsystem classes directly doesn't?" → Simplicity and decoupling. Without Facade: callers must know and coordinate 5 subsystems (auth, payment, inventory, notification, fulfillment) for one business operation. With `OrderFacade.placeOrder()`: caller makes one call; the facade orchestrates all 5 subsystems in the right order. If the subsystem changes (e.g., notification becomes async), only the facade changes — not every caller. The facade is also the natural place to add cross-cutting concerns (logging, error translation, retries).
- "Facade vs Service class — what's the difference?" → Conceptually similar. In practice: Facade typically wraps an existing complex subsystem that you didn't design (legacy code, third-party libraries). Service class is usually the primary application layer class you're building. Facade implies "making something complex simpler to use from the outside." Service class implies "business logic layer." Both are valid — the term matters less than the intent.
- "Can Facade hide a bad design?" → Yes — it's a risk. A Facade that wraps a tightly-coupled, undocumented subsystem doesn't fix the underlying design — it just moves the complexity one layer down. The subsystem is still hard to test, understand, and maintain. Facade is best used as a tactical measure while you improve the subsystem, or when the subsystem is third-party code you can't change. Don't mistake "I added a Facade" for "I fixed the design."
- "How does Facade relate to the Principle of Least Knowledge (Law of Demeter)?" → Facade enforces LoD structurally: callers interact only with the facade, never with the internals of the subsystem. The facade is the single contact point. Without a facade: `order.getPayment().getGateway().charge(amount)` — calling code traverses 3 objects (violates LoD). With facade: `orderFacade.chargeOrder(order, amount)` — the client only knows the facade.
