...
class Splitwise:
    ...
    def add_expense(self, payer, participants, amount, split_type, values):
        if split_type == "EQUAL":
...
class ExpenseManager:
    def __init__(self):
        self._expenses = []   # list of Expense
        self._balance_sheet = {}  # dict[str, dict[str, float]]: user_id → (owed_user_id → amount)

    def add_expense(self, amount, paid_by, splits):
    ...
    def _print_balance(self, user1, user2, amount):
        if amount < 0:
...
class DebtSimplifier:
    ...
    def simplify_debts(self, balance_sheet):
        ...
        creditors = []  # (-net, user_id) — largest creditor first
        debtors = []    # (-abs_debt, user_id) — largest debtor first
...
- "What are the core entities in Splitwise?" → `User`, `Group`, `Expense`, `ExpenseSplit` (how an expense is split per user), `Transaction` (recorded payment between two users). Key operations: add expense, settle up (mark debt as paid), show balances. `Expense` links to `Group`, has a `paid_by` user, total amount, and a list of `ExpenseSplit` records. Balance = sum of all expenses you paid for others minus sum of all expenses others paid for you.
- "How do you compute who owes whom in a group?" → Build a balance map: for each expense, credit the payer, debit each split member. `balance[paid_by] += total - own_share; balance[member] -= member_share` for each member. After processing all expenses: users with positive balance are owed money; negative balance means they owe. To minimize transactions (simplify debts): greedy algorithm — repeatedly pair the most positive with the most negative, create one transaction, reduce both balances. Minimum transactions = N-1 for N people in a balanced group.
- "Equal split vs percentage split vs exact amounts — how do you design for all three?" → Strategy pattern: `SplitStrategy` interface with `calculate(total, participants) → List[ExpenseSplit]`. `EqualSplit.calculate()` divides total by N. `PercentageSplit.calculate()` uses pre-specified percentages (must sum to 100%). `ExactSplit.calculate()` uses pre-specified amounts (must sum to total). Validation is part of each strategy's `calculate()`. `Expense` holds a reference to the chosen strategy. Adding a new split type (e.g., shares-based) = one new class.
- "How do you handle floating point in money calculations?" → Never use `float` for money — floating point representation errors accumulate. Use `Decimal` (Python) or `BigDecimal` (Java) with explicit precision. Store amounts in the smallest currency unit (cents for USD) as integers. `$10.50 → 1050 cents`. All arithmetic in integers; divide only at display time. This eliminates rounding errors entirely.
- "Splitwise simplifies debts across a group. Walk through the algorithm." → Example: A owes B $10, B owes C $10, C owes A $10 — perfectly circular, total owed = $30 but minimum transactions = 0 (it cancels out). Algorithm: compute net balance per person. Separate into creditors (positive) and debtors (negative). Use two pointers: greedily settle the largest debtor against the largest creditor. If creditor balance = debtor balance: one transaction settles both. If unequal: one is fully settled, the other has remainder, continue. O(N log N).
