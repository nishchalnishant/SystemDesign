...
class ParkingSystem:
    def __init__(self, floors, spots_per_floor):
        ...
        self.tickets = {}  # ticket_id → [floor, spot]
        self.pricing_type = "HOURLY"

    def park_car(self, license_plate, vehicle_type):
...
class ParkingSpot:
    def __init__(self, spot_id, spot_type):
        ...
        self.is_free = True
        self.vehicle = None  # Vehicle | None
...
class Level:
    def __init__(self, floor, num_spots):
        ...
        for i in range(num_spots):
            ...
            self.spots.append(ParkingSpot(i, stype))
...
class ParkingLot:
    _instance = None  # ParkingLot | None

    def __new__(cls):
        if cls._instance is None:
            inst = super().__new__(cls)
            inst.levels = []  # list of Level
            cls._instance = inst
...
- "What classes would you design for a parking lot?" → `ParkingLot`, `ParkingFloor`, `ParkingSpot` (with subtypes: `CompactSpot`, `LargeSpot`, `HandicappedSpot`), `Vehicle` (with subtypes: `Car`, `Truck`, `Motorcycle`), `Ticket`, `ParkingFeeCalculator`. Key relationships: `ParkingLot` has many `ParkingFloor`s; each floor has many `ParkingSpot`s; a `Ticket` links a `Vehicle` to a `ParkingSpot` with entry/exit time.
- "How do you handle finding an available spot efficiently?" → Each `ParkingFloor` maintains a count of available spots per type. `ParkingLot.findSpot(vehicleType)` iterates floors and delegates to the floor's `getAvailableSpot(vehicleType)`. Floor maintains a `Set<ParkingSpot>` of available spots per type — O(1) retrieval. When a spot is taken: remove from the set. When freed: add back. Don't scan all spots on every request.
- "The same parking lot class handles `findSpot()`, `calculateFee()`, and `processPayment()`. What's wrong?" → SRP violation — three responsibilities. Refactor: `ParkingSpotFinder` (spot search strategy), `FeeCalculator` (pricing rules), `PaymentProcessor` (payment). `ParkingLot` orchestrates them. This also makes fee calculation testable in isolation and allows different pricing strategies (hourly vs. flat rate) via Strategy pattern.
- "How do you handle the multi-floor search — should the parking lot search floor by floor or in parallel?" → Sequential for simplicity (floor 1 → 2 → 3); return first available. If fairness is required (distribute cars across floors): round-robin search starting from a different floor each time. If speed is critical: each floor runs the search in parallel (thread pool), first result wins. The interviewer is checking whether you consider fairness and performance beyond the happy path.
- "A VIP member gets reserved spots. How do you model this?" → `ParkingSpot` gets a `reserved_for` field: `GENERAL`, `VIP`, `HANDICAPPED`. `findSpot(vehicleType, memberType)` filters by both vehicle type and spot reservation status. `VIP` members can use `VIP` and `GENERAL` spots; `GENERAL` members can only use `GENERAL`. This is a filtering rule in the spot-finding logic, not a subclass.
