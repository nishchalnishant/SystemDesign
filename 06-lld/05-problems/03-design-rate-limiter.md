...
class TokenBucketRateLimiter(RateLimiter):
    ...
    def allow_request(self, user_id):
        if user_id not in self._user_buckets:
            self._user_buckets[user_id] = TokenBucket(self._capacity, self._refill_rate)
        return self._user_buckets[user_id].allow()
...
- "Walk me through implementing a token bucket rate limiter in code." → Fields: `capacity` (max tokens), `tokens` (current count), `refill_rate` (tokens/sec), `last_refill_time`. On `allow_request()`: (1) compute elapsed = now - last_refill_time; (2) add tokens = elapsed × refill_rate, cap at capacity; (3) update last_refill_time; (4) if tokens >= 1: decrement tokens, return True (allow); else return False (deny). Thread-safe: wrap all of `allow_request()` in a lock. The lazy refill avoids a background thread.
- "Token bucket vs sliding window counter — which uses more memory?" → Token bucket: O(1) per user (one record per user: tokens + timestamp). Sliding window log: O(N) per user where N = requests in the window (stores each timestamp). Sliding window counter: O(W) per user where W = number of sub-windows (small constant, e.g., 60 for minute-resolution). For 10M users: token bucket = manageable; sliding window log = potentially billions of timestamps in memory. Memory efficiency strongly favors token bucket.
- "How do you implement this in Redis to handle distributed rate limiting across 10 servers?" → Redis atomic Lua script: `local tokens = redis.call('GET', key); ... redis.call('SET', key, new_tokens); return allow`. The entire check-and-update runs as one atomic Redis operation — no race conditions between the GET and SET. Each app server calls the same Redis key; Redis serializes access. Latency: one Redis round-trip (~1ms). Alternatively: Redis INCR on a per-window key with EXPIRE — simpler but less smooth (fixed window, not token bucket).
- "A user makes 10 requests per second. Your rate limit is 5/second. How quickly does the token bucket drain?" → If capacity = 10, rate = 5 tokens/sec: at 10 requests/sec (net consumption = 10/sec, refill = 5/sec), the bucket drains at 5 tokens/sec. Starting at full (10 tokens): drained in 2 seconds. After that: 5 requests/sec allowed (equilibrium = refill rate). This burst absorption is the key advantage of token bucket over a fixed window — the user can burst up to capacity, then is throttled to the steady-state rate.
