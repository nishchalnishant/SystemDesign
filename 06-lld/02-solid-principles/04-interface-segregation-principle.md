...
class BasicPrinter(MultiFunctionPrinter):
    ...
    def staple(self, d):
        raise NotImplementedError("No stapler")
```
...
class BasicPrinter(Printable, Scannable):
    def print(self, d): ...  # Print logic
    def scan(self, d): ...   # Scan logic
...
class OrderService(ABC):
    ...
    @abstractmethod
    def export_to_csv(self, orders): ...   # Report concern
    @abstractmethod
    def send_confirmation_email(self, order): ...  # Notification concern
...
- "What's the problem with a 'fat' interface? Give a concrete example." → Fat interface: `Machine` with `print()`, `scan()`, `fax()`, `staple()`. An `OldPrinter` that can only print must implement `scan()` and `fax()` as empty methods or exception-throwers — violating LSP and confusing callers. Callers holding a `Machine` reference don't know which methods actually work. ISP fix: split into `Printer`, `Scanner`, `Fax`. `OldPrinter implements Printer` only — honest about capabilities.
- "How do you decide where to split a fat interface?" → Split along actor lines (same as SRP): who calls these methods? A `UserService` with `createUser()`, `deleteUser()`, `getUserReport()`, `getUserAuditLog()` — the admin UI calls the first two; the reporting dashboard calls the last two. Split into `UserManagement` and `UserReporting` interfaces. Each caller depends only on what it uses.
- "ISP says 'clients should not be forced to depend on methods they do not use'. But interfaces are free — why does it matter?" → The cost is coupling. A class implementing a fat interface must be recompiled whenever any method in the interface changes — even methods it doesn't implement. In a compiled language (Java), a change to `fax()` forces all `Machine` implementors to recompile. In a microservice context: a fat API contract means adding one new endpoint version requires all consumers to update their client SDK. ISP minimizes the blast radius of changes.
- "How does ISP relate to the Single Responsibility Principle?" → ISP is SRP applied to interfaces. SRP: a class should have one reason to change. ISP: an interface should have one reason to change — callers should only depend on the subset of methods they use. Both principles reduce coupling and change propagation. Violating ISP usually means you're also violating SRP at the interface level.
